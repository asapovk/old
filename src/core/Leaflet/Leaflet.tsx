...
export default forwardRef((props: Types.Props, ref) => {
  ...
  const defaultItem: number | null =
    (props.default || props.default === 0) && props.data[props.default]
      ? props.default
      : null;
  ...
  return (
    <Flexbox css={styles.container} className={props.className}>
      ...
      {(state.opened.length > 1 || state.choose) && !browser.isMobile && (
        <LeftBar
          styles={styles}
      ...
      <Flexbox css={styles.items}>
        {(!browser.isMobile || !state.opened.length) && (
          <Menu
            ...
            left={
              browser.isMobile || (state.opened.length === 1 && !state.choose)
            }
            styles={styles}
