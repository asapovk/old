...
exports.default = (function (props) {
    ...
    if (groupKey && (Array.isArray(groups) && groups.length > 0)) {
        return (core_1.jsx(react_1.Fragment, null, groups.map(function (group, index) { return (core_1.jsx(react_1.Fragment, { key: group.value + "-" + index },
            ...
            data.filter(function (row) { return row.groupId === group.value; }).map(function (row, index) { return (core_1.jsx(_1.Row, { key: (JSON.stringify(columns) + group.value.toString() + currentPage.toString() + index).hashCode(), row: row, columns: columns, expandForm: expandForm })); }))); })));
    }
    return (core_1.jsx(react_1.Fragment, null, data.map(function (row, index) { return (core_1.jsx(_1.Row, { key: (JSON.stringify(columns) + currentPage.toString() + index).hashCode(), row: row, columns: columns, expandForm: expandForm })); })));
});
